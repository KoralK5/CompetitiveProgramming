...
int dfs(int x, int y, int time, int coins) {
	...
	ckmax(best, dfs(x, y, time+1, coins));
	if (x+1 <= n) {
...
void solve() {
	...
	dp.clear();
	dp.resize(m+1, vector<vi>(n+1, vi(n+1, -1e9)));
