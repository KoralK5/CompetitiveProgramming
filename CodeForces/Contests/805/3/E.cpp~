...
//  greedily create two sets (use maps)

// check for the NO case
// 		cycle of 3
...
bool dfs(int node, int depth) {
	...
	trav (neigh, graph[node]) {
		if (vis[neigh]) {
			if (vis[neigh]%2 != depth%2) continue;
			else return false;
		}
		if (!dfs(neigh, depth+1)) return false;
	}
	return true;
}
...
void solve() {
	...
	FOR (i, 0, n) {
		...
		freq[a]++;
		freq[b]++;
		if (a==b || freq[a] == 3 || freq[b] == 3) ans = false;
	}
