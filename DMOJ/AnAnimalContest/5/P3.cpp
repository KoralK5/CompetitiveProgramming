...
void dfs(int node, int mn, int mx) {
	...
	FOR (i, 0, size) {
		...
		if (i==0) {
			...
			dfs(bp[node][i].fir, max(mn, (bp[node][i].sec+bp[node][i-1].sec+1)/2), mx);
		}
		else {
			dfs(bp[node][i].fir, max(mn, (bp[node][i].sec+bp[node][i-1].sec+1)/2), min(mx, (bp[node][i].sec+bp[node][i+1].sec)/2));
		}
	}
}
