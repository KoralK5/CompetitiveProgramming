...
void print(vector<pair<int, pair<char, char>>> &a) {
...
void solve() {
	...
	// print(a);
	unordered_map<char, int> dp; FOR (i, 0, 26) dp['a'+i] = 0;
	FOR (i, 0, n) {
		...
		pair<char, int> j = make_pair(end, dp[end]);
		int tempScore = temp + j.sec + score + (j.fir==beg && j.sec>0);

		int res = dp[end];
		trav (j, dp) {
			if (j.fir == end) continue;
			res = max(res, j.sec + score + (j.fir==beg && j.sec>0));
		}
		dp[end] = max(res, tempScore);

		// trav (i, dp) if (i.sec) cout << i.fir << ' ' << i.sec << nl;
	}
	int ans=0;
	trav (i, dp) ans = max(ans, i.sec);
	cout << ans << nl;
}
