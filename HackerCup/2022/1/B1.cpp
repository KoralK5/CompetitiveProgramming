...
void solve() {
	...
	FOR (i, 0, n) cin >> trees[i].fir >> trees[i].sec;

	int q; cin >> q;
	vpi wells(q);
	FOR (i, 0, q) cin >> wells[i].fir >> wells[i].sec;

	modnum<MOD> xTrees = 0, yTrees = 0, xTreesC = 0, yTreesC = 0;
	trav (i, trees) {
		modnum<MOD> xv = i.fir;
		modnum<MOD> yv = i.sec;
		xTrees += pow(xv, 2);
	...
	ans += xTrees*q + xWells*n + yTrees*q + yWells*n;
